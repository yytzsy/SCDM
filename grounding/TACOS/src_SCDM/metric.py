...
def analysis_iou(result, epoch, logging):
    ...
    for i in range(len(result)):
        ...
        # logging.info('video duration: = {:f}'.format(video_duration))
        # logging.info(str(ground_truth_interval))
        # logging.info('----------------------')
        # for idx,predict_interval in enumerate(predict_list):
        #     logging.info(str(predict_interval)+'    '+str(predict_score_list[idx]))
        # logging.info('----------------------')
