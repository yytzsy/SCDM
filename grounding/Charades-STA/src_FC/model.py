...
class SSAD_MLP(object):
    ...
    def fuse_multimodal_feature(self, video_fts, sentence_fts, is_training = False, reuse = False, name='default'):
        with tf.variable_scope('fuse_multimodal_feature'+name,reuse=reuse) as scope:
            # video_fts: b,1,1024,556
            # sentence_fts: b,512
            sq_video_fts = tf.squeeze(video_fts,1) # b,1024,556
            ...
            fused_fts = tf.layers.dense(concat_fts_reshape,self.options['dim_hidden']*2,activation=tf.nn.relu,use_bias=True,reuse=reuse,name='dense'+name) # b*1024, 512
    ...
    def network(self, feature_segment, sentence_fts, word_sequence, word_len, batch_size, is_training=False, reuse=False):
        ...
        with tf.variable_scope('base_layer',reuse=reuse) as scope:
            ...
            multimodal_feature_segment = self.fuse_multimodal_feature(feature_segment,sentence_fts,is_training,reuse,name='fts')
            conv1_1 = conv2d(multimodal_feature_segment, self.options['dim_hidden']*2, 512, 1,3, 1, name='conv1')
            conv1 = bn_relu_conv(conv1_1,is_training,512,512,1,3,1,name='conv1_1')
            conv1 = self.fuse_multimodal_feature(conv1,sentence_fts,is_training,reuse,name='conv1')
        ...
        with tf.variable_scope('anchor_fuse',reuse=reuse) as scope:
            anchor1 = self.fuse_multimodal_feature(anchor1,sentence_fts,is_training,reuse,name='anchor1')
            anchor2 = self.fuse_multimodal_feature(anchor2,sentence_fts,is_training,reuse,name='anchor2')
            anchor3 = self.fuse_multimodal_feature(anchor3,sentence_fts,is_training,reuse,name='anchor3')
            anchor4 = self.fuse_multimodal_feature(anchor4,sentence_fts,is_training,reuse,name='anchor4')
            anchor5 = self.fuse_multimodal_feature(anchor5,sentence_fts,is_training,reuse,name='anchor5')
