...
class SSAD_SCM(object):
    ...
    def conv_pool(self,anchor1,sentence_fts,is_training,in_channel,out_channel,h_k,w_k,strides,name_id):
        anchor1 = scm_relu_conv(anchor1, sentence_fts, is_training, in_channel, out_channel, h_k,w_k, strides,name='scm_relu_conv'+str(name_id))
        return anchor1
    ...
    def network(self, feature_segment, sentence_fts, word_sequence, word_len, batch_size, is_training=False, reuse=False):
        ...
        with tf.variable_scope('base_layer',reuse=reuse) as scope:
            ...
            conv1 = scm_relu_conv(conv1_1,sentence_fts,is_training,512,512,1,3,1,name='conv1_1')
        ...
        with tf.variable_scope('anchor_layer',reuse=reuse) as scope:
            anchor1 = self.conv_pool(pool1,sentence_fts,is_training,512,512,1,3,2,1)
            anchor2 = self.conv_pool(anchor1,sentence_fts,is_training,512,512,1,3,2,2)
            anchor3 = self.conv_pool(anchor2,sentence_fts,is_training,512,512,1,3,2,3)
            anchor4 = self.conv_pool(anchor3,sentence_fts,is_training,512,512,1,3,2,4)
            anchor5 = self.conv_pool(anchor4,sentence_fts,is_training,512,512,1,3,2,5)
