...
video_info = pkl.load(open(options['video_info_path']))
...
def make_prepare_path(task):
    ...
    model_save_dir = options['model_save_dir']+sub_dir
    result_save_dir = options['result_save_dir']+sub_dir
...
def get_c3d(video_name):
    return np.array(all_video_fts[video_name]['c3d_fc6_features'])
...
def train(sub_dir, logging, model_save_dir, result_save_dir):
    ...
    for epoch in range(options['max_epochs']):
        ...
        #################################################### test ################################################################################################
        if np.mod(epoch, 1) == 0 and epoch > 60:
            if finetune:
